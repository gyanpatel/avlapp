...
func init() {
	dbfilePath := os.Getenv("filepath")
	_, err := os.Stat(dbfilePath + "sqllite_file.db")

	if err == os.ErrNotExist {
		file, err := os.Create(dbfilePath + "sqllite_file.db")
		defer file.Close()
	...
	db, err := sql.Open("sqlite3", dbfilePath+"sqllite_file.db")
	dbCon = db
	tx, err := dbCon.Begin()
	if err != nil {
