...
func init() {
	_, err := os.Stat("/build/avlapp/data/sqllite_file.db")

	if err == os.ErrNotExist {
		file, err := os.Create("/build/avlapp/data/sqllite_file.db")
		defer file.Close()
	...
	db, err := sql.Open("sqlite3", "/build/avlapp/data/sqllite_file.db")
	dbCon = db
	tx, err := dbCon.Begin()
	if err != nil {
